...
class IncidentService:
    ...
    async def create(self, data: IncidentCreate, created_by: User) -> Incident:
        features = data.network_features.model_dump() if data.network_features else None

        # Generate human-readable external ID (INC-YYYY-NNNNNN)
        ...
        # Queue ML classification if network features provided
        if features:
            try:
                sqs_service.enqueue_incident(str(incident.id), "classify")
            except Exception:
                pass  # Don't fail incident creation if queue is unavailable
    ...
    async def get_stats(self) -> dict:
        total = (await self.db.execute(select(func.count(Incident.id)))).scalar_one()

        by_status = {}
        for status in IncidentStatus:
            count = (
                await self.db.execute(
                    select(func.count(Incident.id)).where(Incident.status == status.value)
                )
            ).scalar_one()
        ...
        for severity in IncidentSeverity:
            count = (
                await self.db.execute(
                    select(func.count(Incident.id)).where(Incident.severity == severity.value)
                )
            ).scalar_one()
